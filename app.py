...
def analyze_resume_with_deepseek(jd_text, resume_text):
    ...
    system_prompt = (
        ...
        "特别声明：本系统在从PDF提取文本时，可能会因为字体编码问题导致简历中出现部分无意义的乱码、错别字或特殊符号。"
        "这完全是系统的技术缺陷导致的，绝非候选人粗心或简历质量问题！"
        "请你在评估时，必须完全忽略所有乱码和奇怪的排版符号，仅根据你能够读懂的正常文本来客观评估，绝对不允许在报告中指责候选人简历乱码或不细心！"
    )
    ...
    # 🌟 只有真正发起网络请求的这一小段，才放进 try...except 保护圈里
    try:
